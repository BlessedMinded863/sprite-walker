/*
 * Fatal Instinct Sprite Walker — 2.5D bone-mapping + IK viewer (three.js).
 *
 * FIX: in the original file this ran as a plain classic <script>, but it
 * called THREE.Scene()/FBXLoader/GLTFLoader/GLTFExporter without ever
 * importing them (those were only imported inside the *other*, ES-module
 * script). A classic script cannot see an ES module's top-level bindings,
 * so this threw "ReferenceError: THREE is not defined" the instant the
 * page loaded and the whole 2.5D viewer never actually ran.
 *
 * It also reached into the sprite-tracer module's private state
 * (fiSketchState, fiSketchDraw, fiSketchCharacterKey, fiDurocCycleAudit)
 * through try/catch + optional-chaining, which silently no-oped every
 * call for the same reason -- those names aren't global either.
 *
 * Fix: this file is now a real ES module that imports THREE/loaders
 * itself, and reaches the sprite-tracer through window.FIProgressiveSketch
 * (app-core.js's own established public bridge for that module -- see
 * the "draw"/"characterKey"/"cycleAudit" entries added there), instead
 * of a try/catch around undefined names.
 */
...
const FI25D_BONE_ALIASES={
  ...
  leftShoulder:['leftshoulder','lshoulder','shoulder_l','mixamorigleftshoulder'],leftUpperArm:['leftarm','leftupperarm','upperarm_l','lupperarm','mixamorigleftarm'],leftForeArm:['leftforearm','leftlowerarm','lowerarm_l','lforearm','mixamorigleftforearm'],leftHand:['lefthand','hand_l','lhand','mixamoriglefthand'],
  rightShoulder:['rightshoulder','rshoulder','shoulder_r','mixamorigrightshoulder'],rightUpperArm:['rightarm','rightupperarm','upperarm_r','rupperarm','mixamorigrightarm'],rightForeArm:['rightforearm','rightlowerarm','lowerarm_r','rforearm','mixamorigrightforearm'],rightHand:['righthand','hand_r','rhand','mixamorigrighthand'],
  leftUpperLeg:['leftupleg','leftthigh','thigh_l','lthigh','mixamorigleftupleg'],leftLowerLeg:['leftleg','leftlowerleg','leftshin','calf_l','lshin','mixamorigleftleg'],leftFoot:['leftfoot','foot_l','lfoot','mixamorigleftfoot'],leftToe:['lefttoebase','lefttoe','toe_l','mixamoriglefttoebase'],
  rightUpperLeg:['rightupleg','rightthigh','thigh_r','rthigh','mixamorigrightupleg'],rightLowerLeg:['rightleg','rightlowerleg','rightshin','calf_r','rshin','mixamorigrightleg'],rightFoot:['rightfoot','foot_r','rfoot','mixamorigrightfoot'],rightToe:['righttoebase','righttoe','toe_r','mixamorigrighttoebase']
};
function fi25dStatus(msg){const e=document.getElementById('fi25dStatus');if(e)e.textContent=msg;}
function fi25dRigStatus(msg){const e=document.getElementById('fi25dRigStatus');if(e)e.textContent=msg;}
function fi25dResize(){const c=document.getElementById('fi25dViewport');if(!c||!FI25D.renderer||!FI25D.camera)return;const w=Math.max(320,c.clientWidth||760),h=Math.max(260,c.clientHeight||430);FI25D.renderer.setSize(w,h,false);FI25D.camera.aspect=w/h;FI25D.camera.updateProjectionMatrix();}
function fi25dNormName(s){return String(s||'').toLowerCase().replace(/[^a-z0-9]/g,'');}
function fi25dAllBones(obj=FI25D.model){const a=[];obj?.traverse(o=>{if(o.isBone)a.push(o)});return a;}
